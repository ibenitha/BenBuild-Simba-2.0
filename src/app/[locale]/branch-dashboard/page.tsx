...
import { ClipboardList, Package, CheckCircle2, XCircle, Clock, User, ChevronDown } from 'lucide-react';
...
const STATUS_CONFIG: Record<OrderStatus, { label: string; color: string; icon: React.ReactNode }> = {
  ...
  completed: { label: 'Completed', color: 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400',          icon: <XCircle className="w-3.5 h-3.5" /> },
};

export default function BranchDashboardPage() {
  const t = useTranslations('branchDashboard');
  const orders = useOperationsStore((s) => s.orders);
  const assignOrder = useOperationsStore((s) => s.assignOrder);
  const markReady = useOperationsStore((s) => s.markReady);
  const completeOrder = useOperationsStore((s) => s.completeOrder);
  const setOutOfStock = useOperationsStore((s) => s.setOutOfStock);
  ...
  useEffect(() => {
    seedBranchStock(simbaBranches.map((b) => b.id));
  }, [seedBranchStock]);
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ...
          { label: 'Preparing', count: counts.accepted, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-900/20' },
          { label: 'Ready', count: counts.ready, color: 'text-green-600', bg: 'bg-green-50 dark:bg-green-900/20' },
          { label: 'Completed', count: counts.completed, color: 'text-slate-500', bg: 'bg-slate-50 dark:bg-slate-800' },
        ].map(s => (
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <section className="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2">
            ...
            <span className="ml-auto text-xs text-slate-400">{branchOrders.length} total</span>
          </div>
                ...
                <div key={order.id} className="px-5 py-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {role === 'manager' && order.status === 'pending' && (
                      <button
                        onClick={() => assignOrder(order.id, 'Staff A')}
        ...
        <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {products.slice(0, 10).map(product => {
              const stock = getBranchStock(branchId, product.id);
              return (
