import Link from 'next/link';
import Image from 'next/image';
import { getTranslations } from 'next-intl/server';
import { categories } from '@/lib/products';

const SOCIAL_LINKS = [
  {
    name: 'Facebook',
    href: 'https://facebook.com/SimbaRwanda',
    label: 'Follow Simba on Facebook',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
      </svg>
    ),
  },
  {
    name: 'Instagram',
    href: 'https://instagram.com/simbarwanda',
    label: 'Follow Simba on Instagram',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>
      </svg>
    ),
  },
  {
    name: 'X',
    href: 'https://x.com/SimbaRwanda',
    label: 'Follow Simba on X',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.737-8.835L1.254 2.25H8.08l4.253 5.622 5.911-5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
      </svg>
    ),
  },
];

export default async function Footer({ locale }: { locale: string }) {
  const t = await getTranslations('footer');
  const tCat = await getTranslations('categories');
  const tNav = await getTranslations('nav');
  return (
    <footer className="bg-slate-900 text-slate-300 mt-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {/* Brand */}
          <div className="md:col-span-1">
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-9 h-9 bg-simba-orange rounded-lg flex items-center justify-center flex-shrink-0">
                <Image src="/simba-logo.png" alt="Simba" width={26} height={26} className="object-contain brightness-0 invert" />
              </div>
              <div>
                <span className="font-black text-lg text-white leading-none block">SIMBA</span>
                <span className="text-[9px] text-slate-500 font-semibold uppercase tracking-widest leading-none">Supermarket</span>
              </div>
            </div>
            <p className="text-sm text-slate-400 leading-relaxed mb-4">
              {t('tagline')}
            </p>
            <div className="space-y-1 text-sm mb-6">
              <p>📞 +250 788 000 000</p>
              <p>📧 hello@simbasupermarket.rw</p>
              <Link href={`/${locale}/locations`} className="flex items-center gap-1 hover:text-simba-orange transition-colors">
                📍 {tNav('storeLocator')}
              </Link>
            </div>

            {/* Social links */}
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-3">{t('followUs')}</p>
              <div className="flex items-center gap-2">
                {SOCIAL_LINKS.map(social => (
                  <a
                    key={social.name}
                    href={social.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label={social.label}
                    className="w-8 h-8 rounded-lg bg-slate-800 hover:bg-simba-orange text-slate-400 hover:text-white flex items-center justify-center transition-all duration-200 hover:scale-110"
                  >
                    {social.icon}
                  </a>
                ))}
              </div>
            </div>
          </div>

          {/* Categories */}
          <div>
            <h3 className="font-semibold text-white mb-4">{t('categories')}</h3>
            <ul className="space-y-2 text-sm">
              {categories.slice(0, 5).map(cat => (
                <li key={cat.id}>
                  <Link href={`/${locale}/products?category=${cat.slug}`} className="hover:text-simba-orange-light transition-colors">
                    {tCat(cat.slug as any)}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* More categories */}
          <div>
            <h3 className="font-semibold text-white mb-4">{t('more')}</h3>
            <ul className="space-y-2 text-sm">
              {categories.slice(5).map(cat => (
                <li key={cat.id}>
                  <Link href={`/${locale}/products?category=${cat.slug}`} className="hover:text-simba-orange-light transition-colors">
                    {tCat(cat.slug as any)}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Info */}
          <div>
            <h3 className="font-semibold text-white mb-4">{t('information')}</h3>
            <ul className="space-y-2 text-sm">
              <li><a href="#" className="hover:text-simba-orange-light transition-colors">{t('about')}</a></li>
              <li><a href="#" className="hover:text-simba-orange-light transition-colors">{t('deliveryPolicy')}</a></li>
              <li><a href="#" className="hover:text-simba-orange-light transition-colors">{t('privacyPolicy')}</a></li>
              <li><a href="#" className="hover:text-simba-orange-light transition-colors">{t('terms')}</a></li>
              <li><a href="#" className="hover:text-simba-orange-light transition-colors">{t('contact')}</a></li>
              <li><Link href={`/${locale}/branch-dashboard`} className="hover:text-simba-orange-light transition-colors">{t('branchDashboard')}</Link></li>
              <li><Link href={`/${locale}/branch-reviews`} className="hover:text-simba-orange-light transition-colors">{t('branchReviews')}</Link></li>
            </ul>
          </div>
        </div>

        <div className="border-t border-slate-800 mt-8 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-sm text-slate-500">
          <p>{t('copyright')}</p>
          <div className="flex items-center gap-4">
            <span>🇷🇼 {t('madeInRwanda')}</span>
            <span>{t('prices')}</span>
          </div>
        </div>
      </div>
    </footer>
  );
}
